...
class ProjectSynchronizeCommandTest extends PHPUnit_Framework_TestCase
{
    ...
    public function execute_method(){
        ...
        $mockService->expects($this->at(5))->method('synchronizePackageVersions')->with($mockProject)->will($this->returnValue([$mockDependencyPackage]));

        $mockPackage->expects($this->any())->method('getName')->will($this->returnValue('testPackageName'));
        $mockPackage->expects($this->any())->method('getMaxVersion')->will($this->returnValue('testPackageVersion'));
        ...
        $mockDependencyProjectPackage->expects($this->at(0))->method('getPackage')->will($this->returnValue($mockDependencyPackage));
        ...
        $mockOutput->expects($this->at(0))->method('writeln')->with('update package testPackageName - testPackageVersion');
        ...
        $mockOutput->expects($this->at(3))->method('writeln')->with('updated outdated package testDependencyPackage');
