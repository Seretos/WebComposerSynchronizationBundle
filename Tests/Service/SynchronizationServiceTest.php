...
class SynchronizationServiceTest extends PHPUnit_Framework_TestCase
{
    ...
    public function synchronizePackages()
    {
        ...
        $this->mockSaveService->expects($this->at(0))->method('buildPackage')->with('test', null)->will($this->returnValue('test1'));
        $this->mockSaveService->expects($this->at(1))->method('buildPackage')->with('test2', null)->will($this->returnValue('test2'));
        $this->mockSaveService->expects($this->at(2))->method('buildPackage')->with('test3', 'repositoryUrl')->will($this->returnValue('test3'));
...
phpunit/phpunit                              5.6.0   5.6.1   The PHP Unit Testing framework.
    '));
        ...
        $mockDriverPackage = $this->getMockBuilder(ProjectPackage::class)->disableOriginalConstructor()->getMock();
        $mockDriverPackage->expects($this->at(0))->method('setMaxVersion')->with('v0.2.4');
        $mockPhpUnitPackage = $this->getMockBuilder(ProjectPackage::class)->disableOriginalConstructor()->getMock();
        $mockPhpUnitPackage->expects($this->at(0))->method('setMaxVersion')->with('5.6.1');

        $mockProjectPackageRepository = $this->getMockBuilder(ProjectPackageRepository::class)->disableOriginalConstructor()->getMock();
        $mockProjectPackageRepository->expects($this->at(0))->method('findOneByProjectAndPackageName')->with($mockProject, 'LimetecBiotechnologies/database/DriverBundle')->will($this->returnValue($mockDriverPackage));
        $mockProjectPackageRepository->expects($this->at(1))->method('findOneByProjectAndPackageName')->with($mockProject, 'phpunit/phpunit')->will($this->returnValue($mockPhpUnitPackage));

        $this->mockEntityManager->expects($this->at(0))->method('getRepository')->will($this->returnValue($mockProjectPackageRepository));
        $this->mockEntityManager->expects($this->at(1))->method('persist')->with($mockDriverPackage);
        $this->mockEntityManager->expects($this->at(2))->method('persist')->with($mockPhpUnitPackage);
        $this->mockEntityManager->expects($this->at(3))->method('flush');
